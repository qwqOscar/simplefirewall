...
void Permit_User(const int *fd, const char *optarg, my_iptable *my_ipt);
void Deny_User(const int *fd, const char *optarg, my_iptable *my_ipt);
...
int main(int argc, char **argv) {
	...
	while(1) {
		...
		switch(opt) {
			...
			case DELETE:
				...
				return 0;			
			case PERMIT:
				Permit_User(&fd, optarg, my_ipt);					// permit the user with a given ID to access the minifirewall table 
				printf("Permission option set\n");
				close(fd);
				free(my_ipt);
				return 0;
			case DENY:
				Deny_User(&fd, optarg, my_ipt);					// permit the user with a given ID to access the minifirewall table 
				printf("Permission option set\n");
				close(fd);
				free(my_ipt);
				return 0;
	...
	if(my_ipt->action != BLOCK && my_ipt->action != UNBLOCK)
	{
		printf("Error: action(whether block or unblock) not specified\n");		
		free(my_ipt);
		exit(EXIT_FAILURE);
	}
	if((my_ipt->ip_rule.bitmask & IS_SIP) && !(my_ipt->ip_rule.bitmask & IS_SMASK))
	{
		printf("Error: srcip specified but srcnetmask not specified\n");		
		free(my_ipt);
		exit(EXIT_FAILURE);
	}
	if(!(my_ipt->ip_rule.bitmask & IS_SIP) && (my_ipt->ip_rule.bitmask & IS_SMASK))
	{
		printf("Error: srcnetmask specified but srcip not specified\n");		
		free(my_ipt);
		exit(EXIT_FAILURE);
	}
	if((my_ipt->ip_rule.bitmask & IS_DIP) && !(my_ipt->ip_rule.bitmask & IS_DMASK))
	{
		printf("Error: destip specified but destnetmask not specified\n");		
		free(my_ipt);
		exit(EXIT_FAILURE);
	}
	if(!(my_ipt->ip_rule.bitmask & IS_DIP) && (my_ipt->ip_rule.bitmask & IS_DMASK))
	{
		printf("Error: destnetmask specified but destip not specified\n");		
		free(my_ipt);
		exit(EXIT_FAILURE);
	}
...
void Set_Net_Mask(const char *optarg, unsigned char is_source, my_iptable *my_ipt) {
	...
	printf("netmask is: %s\n", str1);
	if((netmask->s_addr = inet_addr(str1)) == -1) {
...
void Read_From_Proc(int *fd) {
	my_iptable *my_ipta = (my_iptable *)malloc(100 * sizeof(my_iptable) + 1);
	my_iptable *my_iptb = my_ipta;
	...
	*fd = open("/proc/minifw", O_RDONLY);
	if (read(*fd, my_ipta, 100 * sizeof(my_iptable)) < 0) {
		fprintf(stderr, "Error reading /proc/minifw\n");
		close(*fd);
		free(my_iptb);
		return;
	}
	int num = *(unsigned char*)(my_ipta + 100);
	printf("Total num of rules is %d\n", num);
	for(int i = 0;i < num;i++)
	{
		printf("Rule %d: ", i + 1);
		Parse_Rules(my_ipta);
...
void Parse_Rules(const my_iptable *my_ipt) {
	...
	if (my_ipt->action== BLOCK)
		printf("--action BLOCK \n");
	else if (my_ipt->action == UNBLOCK)
...
void Deny_User(const int *fd, const char *optarg, my_iptable *my_ipt) {
	...
	my_ipt->uid = (uid % UID_MAX);
	printf("deny to minifw granted to uid: %d\n", uid);
	write(*fd, my_ipt, sizeof(my_iptable));
	return;
}
